            ...
            public void run() {
                ...
                if (actionValidation == null || actionValidation.test(gamePlayer)) {
                    if (getCountdown(gamePlayer) <= 0){
                        ...
                        if (actionbar) {
                            ModuleManager.getModule(MessageModule.class).get(gamePlayer, "kit_ability.countdown_is_over").send(MessageType.ACTIONBAR);
                        }
                        this.cancel();
                    }else {
                        if (actionbar) {
                            Utils.countdownTimerBar(gamePlayer.getOnlinePlayer(), name, cooldown, getCountdown(gamePlayer));
                        }
                    }
                }
            }
